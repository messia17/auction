  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        throw new Error(`Server returned status ${response.status} without a JSON response.`);
      }
      ...
      throw new Error(errorData.error || errorData.originalError || 'Search failed');
    }
    ...
    console.warn('Allegro search failed (API & Scraper). Reason:', errMessage);

    // In order for the frontend to show the exact error in logs, we need to throw it,
